...
if ($searchDesignFormContainer.length != 0) {
  console.log(
    "Search user form detected in manage user interface. Binding event handling logic to form elements."
  );
        ...
        for (let index = 0; index < records.length; index++) {
          ...
          if (index % 2 == 0)
            $container = $("<div></div>").addClass(
              " offset-sm-2 col-sm-4 mb-4"
            );
          else $container = $("<div></div>").addClass(" col-sm-4 mb-4");
          let $card = $("<div></div>")
            .addClass("card")
            .attr("style", "width: 18rem;");
          let $cardBody = $("<div></div>").addClass("card-body");
          let $editUserButtonBlock = $("<div></div>").addClass(
            "position-absolute bottom-0 end-0"
          );
          $editUserButtonBlock.append(
            $("<a>Manage</a>")
              .addClass("btn btn-primary")
              .attr("href", "update_user.html?id=" + record.user_id)
          );
          $cardBody.append(
            $("<h5></h5>").addClass("card-title").append(record.fullname)
          );
          $cardBody.append(
            $("<p></p>").addClass("card-text").append(record.email)
          );
          $cardBody.append($editUserButtonBlock);
          if (record.role_name == "admin") {
            $cardBody.append(
              $("<img></img>")
                .attr({ src: "../images/admin.png", widthc: "50" })
                .addClass("text-right")
            );
          } else {
            $cardBody.append(
              $("<img></img>")
                .attr({ src: "../images/user.png", widthc: "50" })
                .addClass("text-right")
            );
          }
  ...
  function clickHandlerForPageButton(event) {
    ...
    let token = sessionStorage.getItem("token");
    console.log(
      "Checking the button page number which raised the click event : ",
      pageNumber
    );
    axios({
      headers: {
      ...
      .then(function (response) {
        ...
        for (let index = 0; index < records.length; index++) {
          ...
          let $card = $("<div></div>")
            ...
            .attr("style", "width: 18rem;");
          let $cardBody = $("<div></div>").addClass("card-body");
          $editUserButtonBlock = $("<div></div>").addClass(
            "position-absolute bottom-0 end-0"
          );
          $editUserButtonBlock.append(
            $("<a>Manage</a>")
