...
if ($searchDesignFormContainer.length != 0) {
    ...
    axios({
      ...
      .then(function (response) {
        ...
        for (let index = 0; index < records.length; index++) {
          ...
          $editDesignButtonBlock.append(
            $("<a>Update</a>")
              .addClass("btn btn-primary")
              .attr("href", "update_design.html?id=" + record.file_id)
          );
          $cardBody.append(
            $("<h5></h5>").addClass("card-title").append(record.design_title)
          );
          $cardBody.append(
            $("<p></p>").addClass("card-text").append(record.design_description)
          );
  ...
  function clickHandlerForPageButton(event) {
    ...
    axios({
      ...
      url:
        baseUrl +
        "/api/user/process-search-design/" +
        pageNumber +
        "/" +
        searchInput,
    })
