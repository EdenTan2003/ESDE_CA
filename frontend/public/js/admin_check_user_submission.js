...
if ($searchUserFormContainer.length != 0) {
  console.log(
    "Search user form detected in user manage submission interface. Binding event handling logic to form elements."
  );
        ...
        for (let index = 0; index < records.length; index++) {
          ...
          if (index % 2 == 0)
            $container = $("<div></div>").addClass(
              " offset-sm-2 col-sm-3 mb-4"
            );
          else
            $container = $("<div></div>").addClass(
              " offset-sm-1 col-sm-3 mb-4"
            );
          let $card = $("<div></div>").addClass("card text-center h-100");
          $card.append(
            $("<img></img>")
              .addClass("card-img-top")
              .addClass("app_thumbnail")
              .attr("src", record.cloudinary_url)
          );
          let $cardBody = $("<div></div>").addClass("card-body");
          $editDesignButtonBlock = $("<div></div>").addClass(
            "position-absolute bottom-0 end-0"
          );
          $cardBody.append(
            $("<h5></h5>").addClass("card-title").html(record.design_title)
          );
          $cardBody.append(
            $("<p></p>").addClass("card-text").html(record.design_description)
          );
  ...
  function clickHandlerForPageButton(event) {
    ...
    axios({
      ...
      url:
        baseUrl +
        "/api/user/process-search-user-design/" +
        pageNumber +
        "/" +
        searchInput,
    })
      .then(function (response) {
        ...
        for (let index = 0; index < records.length; index++) {
          ...
          let $cardBody = $("<div></div>").addClass("card-body");
          let $editDesignButtonBlock = $("<div></div>").addClass(
            "position-absolute bottom-0 end-0"
          );
          $cardBody.append(
            $("<h5></h5>").addClass("card-title").html(record.design_title)
        ...
        for (let count = 1; count <= totalPages; count++) {
          $pageButtonContainer.append(
            $('<button class="btn btn-primary btn-sm"></button>')
              .text(count)
              .on("click", clickHandlerForPageButton)
          );
        }
      })
      .catch(function (response) {
