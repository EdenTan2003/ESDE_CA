  ...
  fileDataManager.uploadFile(file, async function (error, result) {
    ...
    try {
      if (error) {
        ...
        console.log(
          "Inspect result variable inside fileDataManager.uploadFile code"
        );
        console.log(result);
        if (result) {
...
exports.processGetSubmissionData = async (req, res, next) => {
  ...
  try {
    ...
    if (results) {
      ...
      // Implementing the sanitizeResult to prevent XSS
      jsonResult = validateFn.sanitizeResult(jsonResult);
...
exports.processUpdateOneUser = async (req, res, next) => {
  ...
  try {
    ...
    let message = "Unable to complete the update operation.";
    return res.status(500).json({
      error: error,
      message: message,
    });
...
exports.processSendInvitation = async (req, res, next) => {
  ...
  try {
    ...
    if (results) {
      ...
      // Implementing the sanitizeResult to prevent XSS
      validateFn.sanitizeResult(jsonResult);
      return res.status(200).json(jsonResult);
    }
  } catch (error) {
    console.log(error);
